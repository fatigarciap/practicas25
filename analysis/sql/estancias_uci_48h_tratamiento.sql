...
PriorTreatment AS (
  SELECT
    ...
    s.interpretation,
    CASE
      WHEN COUNT(p.starttime) > 0 THEN 1
      ELSE 0
  ...
  ON
    ...
    AND LOWER(p.drug) IN (
      ...
      'trimethoprim-sulfamethoxazole', 'linezolid', 'daptomycin', 'oxacillin', 'vancomycin'
    )
  GROUP BY
    s.stay_id,
    s.hadm_id,
...
PostTreatment AS (
  SELECT
    ...
    CASE
      WHEN COUNT(p2.starttime) > 0 THEN 1
      ELSE 0
    END AS has_post_treatment
  FROM
    PriorTreatment p
  LEFT JOIN
    `physionet-data.mimiciv_3_1_hosp.prescriptions` p2
  ON
    p.hadm_id = p2.hadm_id
...
SELECT
  COUNT(DISTINCT stay_id) AS estancias_distintas_sin_tratamiento_previo_y_posterior_48h
FROM
  FilteredStays
WHERE
  has_prior_treatment = 0
  AND has_post_treatment = 1
